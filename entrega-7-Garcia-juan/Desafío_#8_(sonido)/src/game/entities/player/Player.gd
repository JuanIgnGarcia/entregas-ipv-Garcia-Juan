...
var hp_damage_tween: SceneTreeTween

func _animate_hp_damage() -> void:
	if hp_damage_tween:
		hp_damage_tween.kill()
	
	body_pivot.modulate.r = Color.white.r
	var target_r: float = 1.0 + 2.0 * (1.0 - (float(hp) / max_hp))
	hp_damage_tween = create_tween()
	hp_damage_tween.set_loops()
	hp_damage_tween.tween_property(body_pivot, "modulate:r", target_r, 1.0).set_trans(Tween.TRANS_SINE)
	hp_damage_tween.tween_property(body_pivot, "modulate:r", 1.0, 1.0).set_trans(Tween.TRANS_SINE)
